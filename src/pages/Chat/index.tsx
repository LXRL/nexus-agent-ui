...
import { Input, Button, Form, Spin, Avatar, Empty, message as antdMessage, Popover } from 'antd';
import { SendOutlined, UserOutlined, RobotOutlined } from '@ant-design/icons';
import type { RootState, AppDispatch } from '../../store/store';
import type { Message, MessageContentBlock } from '../../types';
import { setActiveConvId, updateConversation } from '../../store/slices/conversationSlice';
import { getConversationHistory, performAttributionAnalysis } from '../../api/conversationService';
...
const ChatPage: React.FC = () => {
  ...
  const activeConvId = useSelector((state: RootState) => state.conversation.activeConvId);
  ...
  const handlePerformAttribution = async () => {
    ...
    setIsLoading(true);
    try {
        const result = await performAttributionAnalysis(activeConvId, '请详细分析一下原因');
  ...
  const handleSendMessage = async ({ question }: { question:string }) => {
    ...
    eventSource.onmessage = (event) => {
      ...
      if (type === 'done') {
        ...
        if (newConvId && newConvTitle) {
          dispatch(updateConversation({ id: newConvId, title: newConvTitle, updated_at: new Date().toISOString() }));
          dispatch(setActiveConvId(newConvId));
        }
        return;
      }
      
      const newBlock: MessageContentBlock | null = payload ? JSON.parse(payload) : null;
      if (!newBlock) return;
    ...
    eventSource.onerror = () => {
      const errorBlock: MessageContentBlock = { type: 'error', error: '与服务器的连接发生错误。' };
      setMessages(prev =>
        prev.map(msg => 
          msg.id === assistantMessageId ? { ...msg, content: [...msg.content, errorBlock] } : msg
        )
      );
      eventSource.close();
      setIsLoading(false);
      ...
      <div className={styles.chatPage}>
        <div className={styles.messagesContainer}>
          {messages.length === 0 && !isLoading ? (
            <Empty description="开始一段新的分析对话吧！" style={{marginTop: '20vh'}}/>
          ) : (
            messages.map(msg => (
              <div key={msg.id} className={`${styles.messageWrapper} ${msg.role === 'user' ? styles.userMessage : styles.assistantMessage}`}>
                <Avatar icon={msg.role === 'user' ? <UserOutlined /> : <RobotOutlined />} style={{ alignSelf: msg.role === 'user' ? 'flex-end' : 'flex-start' }}/>
          ...
          {isLoading && messages.length > 0 && <Spin style={{ alignSelf: 'center', marginTop: 12 }}/>}
          <div ref={messagesEndRef} />
        </div>
        <div className={styles.inputArea}>
          <Popover content="在一次分析结束后，点击结果下方的“归因分析”按钮来探索原因。" trigger="focus">
            <Form form={form} onFinish={handleSendMessage} layout="inline" style={{width: '100%'}}>
              <Form.Item name="question" style={{flex: 1}}>
                <Input size="large" placeholder="请输入你的问题..." disabled={isLoading} />
              </Form.Item>
              <Form.Item>
                <Button type="primary" htmlType="submit" size="large" icon={<SendOutlined />} disabled={isLoading}/>
              </Form.Item>
            </Form>>
          </Popover>
        </div>
